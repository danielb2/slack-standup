...
var $home = util.format("%s", process.env.HOME).trim().replace(/\/+$/g, "");
var standup_dir = util.format("%s/%s", $home, ".standup");
var prefs_file = util.format("%s/%s", $home, ".slackrc.json");
...
if (!dirIsWritable(standup_dir)) {
  console.log(util.format("Error: required directory '%s' not found or not writable!", standup_dir));
  process.exit(1);
}

// if not ok - exit
if (!fileIsReadable(prefs_file)) {
  console.log(util.format("Error: required preference file '%s' not found or not readable!", prefs_file));
  process.exit(1);
}
...
try {
  ...
  console.log(util.format("Error# parsing preference file :%s", e.message).replace(/:/, "'").replace(/:/, "'").replace(/#/, ":"));
  process.exit(1);
}
...
Object.keys(config)
  .map(function(prop) {
    ...
    if (!util.isUndefined(process.env[prop.toUpperCase()])) {
      config[prop] = process.env[prop.toUpperCase()];
    }

    // top level of rc file
    if (!util.isUndefined(prefs[prop])) {
      config[prop] = prefs[prop];
    }
    // standup level of rc file
    if (!util.isUndefined(prefs.standup) && !util.isUndefined(prefs.standup[prop])) {
      config[prop] = prefs.standup[prop];
    }

    // if not set ~ get out
    if (config[prop] === "") {
      console.log(util.format("Error: required preference '%s' not found in file '%s' or as uppercase in ENV!", prop, prefs_file));
      process.exit(1);
    }
    return prop;
  });
...
for (var i = 0, d, ds, standup_file; i < 7; i++) {
  if (!config.prev_standup) {
    ...
    standup_file = util.format("%s/standup.%s.txt", standup_dir, ds);
...
if (config.prev_standup && config.prev_today) {
  var standup_json = config.standup.replace(/\.txt$/, ".json");
  if (fileIsReadable(standup_json)) {
    try {
      config.standup_json = require(standup_json);
    }
    catch(e) {
      config.standup_json = null;
    }
  }
}
