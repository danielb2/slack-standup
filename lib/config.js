...
for (let i = 0;  i < 7; ++i) {
    if (!config.prev_standup) {
        ...
        const standup_file = Util.format('%s/standup.%s.json', internals.standup_dir, ds);
        ...
        if (i === 0) {
            ...
            config.standup_ts_file = standup_file.replace(/\.json/, Util.format('.%s.json', config.channel.replace(/[^a-zA-Z0-9-]/g, '')));
        }
    }
}
