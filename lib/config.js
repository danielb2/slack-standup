...
if (!File.hasConfig) {
    console.log('Error: required preference file not found or not readable!');
}
...
for (let i = 0;  i < 7; ++i) {
    if (!config.prev_standup) {
        ...
        if (i === 0) {
            ...
            config.standup_ts_file = `${standup_file}.${config.channel.replace(/[^a-zA-Z0-9-]/g, '')}`;
        }
    }
}
...
if (config.prev_standup && config.prev_today) {
    if (File.exists(config.standup_ts_file)) {
        try {
